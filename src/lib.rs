...
impl Display for TypeError {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    write!(f, "Invalid Font Awesome icon type: must be one of brands, regular, or solid")
  }
}
...
impl Display for NameError {
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    write!(f, "Invalid Font Awesome icon name: visit https://fontawesome.com/icons?d=gallery&m=free to see valid names")
  }
}
...
mod tests {
    #[test]
    fn it_works() {
        assert!(crate::svg(crate::Type::Regular, "cog").is_ok());
        assert!(crate::svg(crate::Type::Regular, "snuffleupigus").is_err());
    }
}
