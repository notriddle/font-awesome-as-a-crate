...
fn write_fontawesome_sprite() {
    ...
    dest_file.write("fn fontawesome_svg(dir:&str,file:&str)->&'static str{match(dir,file){".as_bytes()).expect("fontawesome fn write");
    for dirname in &["brands", "regular", "solid"] {
        ...
        for file in dir {
          ...
          let filename = file.file_name().into_string().expect("fontawesome filenames are unicode");
          let mut file = File::open(file.path()).expect("fontawesome file access");
          data.clear();
          file.read_to_string(&mut data).expect("fontawesome file read");
          // if this assert goes off, add more hashes here and in the format! below
          assert!(data.find("###").is_none(), "file {} breaks raw string", filename);
          dest_file.write(format!(
              r####"("{dirname}","{filename}")=>r###"{data}"###,"####,
              data = data,
              dirname = dirname,
              filename = filename.replace(".svg", ""),
          ).as_bytes()).expect("write fontawesome file");
        }
    }
    dest_file.write("_=>panic!(\"missing image file\")}}".as_bytes()).expect("fontawesome fn write");
}
